...
This repo is also the single point of fault-tolerance failure for the whole family: it is the only load balancer in front of every backend service's ASG. A change here to health-check thresholds, timeouts, connection draining, or spot-interrupt handling is a family-wide reliability change, not a local tweak — treat it with the same scrutiny you would give a shared library used by every service, because every service inherits this repo's health-check and failover behavior whether or not its own code changes.
